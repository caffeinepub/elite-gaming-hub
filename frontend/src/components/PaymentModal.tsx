...
import { X, Upload, Copy, CheckCircle, ImageIcon, CreditCard } from 'lucide-react';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
import { RegistrationData, PaymentData } from '@/types/registration';
      ...
      >
        ...
        >
          ...
          <div className="px-6 py-5 space-y-5">
            {/* Entry Fee */}
            <div className="bg-dark-bg rounded-md px-4 py-3 flex items-center justify-between"
              ...
              <span className="font-orbitron font-bold text-xl text-neon-red-bright neon-text">₹{entryFee}</span>
            </div>
            ...
            <div className="space-y-2">
              ...
              >
                <div className="flex-1 min-w-0">
                  <p className="font-rajdhani text-xs text-muted-foreground uppercase tracking-wider mb-0.5">UPI ID</p>
                  <p className="font-orbitron font-bold text-sm text-foreground truncate">{UPI_ID}</p>
                </div>
                <button
              ...
              <p className="font-rajdhani text-xs text-muted-foreground/70 tracking-wide">
                Send ₹{entryFee} to the UPI ID above, then upload the payment screenshot below.
              </p>
            </div>

            {/* Player Info Summary */}
            {registrationData && (
              <div className="bg-dark-bg rounded-md px-4 py-3 space-y-1"
                style={{ border: '1px solid oklch(0.55 0.22 25 / 0.15)' }}>
                <p className="font-rajdhani font-semibold text-xs uppercase tracking-widest text-muted-foreground mb-2">Player Info</p>
                <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                  <div>
                    <p className="font-rajdhani text-xs text-muted-foreground/60 uppercase tracking-wider">Name</p>
                    <p className="font-rajdhani font-semibold text-sm text-foreground">{registrationData.inGameName}</p>
                  </div>
                  <div>
                    <p className="font-rajdhani text-xs text-muted-foreground/60 uppercase tracking-wider">Player ID</p>
                    <p className="font-rajdhani font-semibold text-sm text-foreground">{registrationData.playerId}</p>
                  </div>
                </div>
              </div>
            )}
              ...
              {previewUrl ? (
                ...
                >
                  ...
                  <div className="text-center">
                    <p className="font-rajdhani font-semibold text-sm text-foreground">Upload Screenshot</p>
                    <p className="font-rajdhani text-xs text-muted-foreground/60 mt-0.5">PNG, JPG, JPEG accepted</p>
                  </div>
                </button>
              )}
